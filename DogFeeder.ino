...
#ifdef DHT_H
  TempC = dht.readTemperature() + 0.5f;
#else
  TempC = DS3231_GetTemperature() + 0.5f;
#endif
...
void DogFeeder(void)
{
  ...
  if (((hrs == FeedHours1) && (min == FeedMins1)) || ((hrs == FeedHours2) && (min == FeedMins2)) || (feed_dog))
  {
    if (!dog_fed)
    {
      dog_fed = true;
